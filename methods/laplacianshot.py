...
from utils.paddle_utils import Logger, get_one_hot
...
class LAPLACIAN_SHOT(object):
    ...
    def record_info(self, acc, ent_energy, new_time, criterions, preds):
        """
        inputs:
    ...
    def get_logs(self):
        ...
        self.outs = [torch.stack(self.outs)] # To keep compatible format with PADDLE and other methods
    ...
    def bound_update(self, unary, kernel, bound_lambda, y_s, y_q, task_i, bound_iteration=20, batch=False):
        ...
        for i in range(bound_iteration):
            ...
            if (i > 1 and (abs(E - oldE) <= 1e-6 * abs(oldE))):
                ...
                if self.maj_vote:
                    out_maj = majority_or_original(torch.tensor([out]).to(torch.device('cpu')))
                    acc = (torch.from_numpy(y_q[task_i]) == out_maj).float().mean(1, keepdim=True)
                else:
                    acc = (torch.from_numpy(y_q[task_i]) == torch.from_numpy(np.expand_dims(out,0))).float().mean(1,keepdim=True)
                ...
                oldE = E.copy()
                if self.maj_vote:
                    out_maj = majority_or_original(torch.tensor(np.expand_dims(out,0)).to(torch.device('cpu')))
                    acc = (torch.from_numpy(y_q[task_i]) == out_maj).float().mean(1, keepdim=True)
                else:
                    acc = (torch.from_numpy(y_q[task_i]) == torch.from_numpy(np.expand_dims(out,0))).float().mean(1,keepdim=True)
            t0 = time.time()
        ...
        if self.maj_vote:
            return out_maj[0], acc, E_list, timestamps
        else:
            return torch.from_numpy(out), acc, E_list, timestamps
    ...
    def run_method(self, support, query, y_s, y_q):
        ...
        for i in tqdm(range(self.number_tasks)):
            ...
            self.record_info(acc=acc_list, ent_energy=ent_energy,
                             new_time=t1 - t0, criterions=[0], preds=preds)